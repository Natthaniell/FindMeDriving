    ...
    .directive('uiSwitch', ['$window', '$timeout','$log', '$parse', function($window, $timeout, $log, $parse) {
        ...
        function linkSwitchery(scope, elem, attrs, ngModel) {
            ...
            function initializeSwitch() {
                ...
                /**
                 * Switchery - extend default functionality
                 * Allow dual binding in what every way model will change.
                 * It will update UI state no matter what has changed the model.
                 * Radek
                 */
                scope.$watch('initValue', function(newValue, oldValue){
                    if(angular.isDefined(switcher)){
                        if(switcher.isChecked() !== newValue){
                            switcher.setPosition(newValue);
                        }
                    }
                });
